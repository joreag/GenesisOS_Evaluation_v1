...
impl KmictKernel {
    pub fn new(db_conn: rusqlite::Connection, app_handle: tauri::AppHandle) -> Self { // The function starts here
        let (tx, rx) = channel();
        let db_conn = database::initialize_database(&app_handle);
        ...
        // The function RETURNS the struct here
        KmictKernel {
            message_tx: tx,
            message_rx: rx,
    ...
    pub fn pump_message_bus(&mut self) -> Vec<String> {
        ...
        while let Ok(msg) = self.message_rx.try_recv() {
            ...
            // --- THE DEFINITIVE KERNEL ROUTER v3 ---
            let (target_mdo_blueprint, context_prefix) = 
                if let Some(mdo) = self.mdo_inventory.get(&msg.target_id) {
                    // Case 1: Direct call to a loaded MDO blueprint.
                    let prefix = if msg.target_id == "mdo_jarvits" {
                        "mdo_jarvits_".to_string()
                    } else {
                        "".to_string() // Ledger, Scheduler, Identity etc. are global
                    };
                    (Some(mdo.clone()), prefix)
                } else if self.world_state.get(&format!("{}_object_id", msg.target_id)).is_some() {
                    // Case 2: It's a data context. Determine the blueprint and prefix.
                    let (blueprint, prefix) = if msg.target_id.starts_with("proc_") {
                        (self.mdo_inventory.get("mdo_process").cloned(), format!("{}_", msg.target_id))
                    } else { 
                        // If it's not a process, assume it's a file data context
                        (self.mdo_inventory.get("mdo_file").cloned(), format!("{}_", msg.target_id))
                    };
                    (blueprint, prefix)
                } else {
                    (None, "".to_string())
            ...
            if let Some(target_mdo) = target_mdo_blueprint {
                ...
                // HYDRATION (Read from specific to generic)
                for generic_var_name in target_mdo.map_block.variables.keys() {
                    let specific_var_name = format!("{}{}", context_prefix, generic_var_name);
                    
                    if !inputs.contains_key(generic_var_name) {
                        // First, try to load the specific, prefixed variable
                        if let Some(val) = self.world_state.get(&specific_var_name) {
                            inputs.insert(generic_var_name.clone(), val.clone());
                        // If that fails, try to load a global variable of the same name
                        } else if let Some(val) = self.world_state.get(generic_var_name) {
                             inputs.insert(generic_var_name.clone(), val.clone());
                        }
                    }
                }
                ...
                match runtime.run_mict_cycle(&target_mdo, inputs) {
                    Ok(emissions) => {
                        ...
                        for transform in emissions {
                            match transform {
                                mdo_types::TransformStatement::Emit { payload, .. } => {
                                    let mut state_updates = HashMap::new();
                                    let mut ipc_directives = HashMap::new();

                                    // Sort emissions into State vs IPC Directives
                                    ...
                                    // 1. Commit State Mutations (WRITE-BACK LOGIC)
                                    for (k, v) in state_updates {
                                        let save_key = format!("{}{}", context_prefix, k);
                                        log.push(format!("    [STATE MUTATION] {} = {:?}", save_key, v));
                                        if let Err(e) = self.world_state.insert(save_key.clone(), v) {
                                            log.push(format!("    [KERNEL PANIC] Memory Write Failed: {}", e));
                                        }
                                    }

                                    // 2. Dispatch IPC Messages & Hardware Interrupts
                                    if !ipc_directives.is_empty() {
                                        
                                        let mut is_hardware_call = false;

                                                                                // --- BARE-METAL & HOST OS INTERCEPTS ---
                                        if let Some(RuntimeVal::String(hw_target)) = ipc_directives.get("_target_hardware_interrupt") {
                                            is_hardware_call = true;
                                            if hw_target == "HOST_OS" {
                                                if let Some(RuntimeVal::String(cmd)) = ipc_directives.get("_payload") {
                                                    log.push(format!("    [HOST OS EXEC] Validated command: {}", cmd));
                                                    // In a real terminal, we'd stream this output back to the UI.
                                                    // For now, logging proves the gatekeeping worked.
                                                    match std::process::Command::new("bash").arg("-c").arg(cmd).output() {
                                                        Ok(out) => {
                                                            let stdout = String::from_utf8_lossy(&out.stdout).to_string();
                                                            let stderr = String::from_utf8_lossy(&out.stderr).to_string();
                                                            log.push(format!("    [HOST OS STDOUT] -> {}", stdout.trim()));
                                                            if !stderr.is_empty() {
                                                                log.push(format!("    [HOST OS STDERR] -> {}", stderr.trim()));
                                                            }
                                                        },
                                                        Err(e) => log.push(format!("    [HOST OS ERROR] -> {}", e)),
                                                    }
                                                }
                                            }
                                            // You can add other hardware intercepts like "NIC" or "I/O_PORT" here
                                        }

                                        // --- INTERCEPTOR A: Bare-Metal I/O Port (for UartDriver.mdo) ---
                                        if let Some(RuntimeVal::Int(port)) = ipc_directives.get("_target_hardware_io_port") {
                                            is_hardware_call = true;
                                            if let Some(RuntimeVal::String(cmd)) = ipc_directives.get("_command") {
                                                if let Some(RuntimeVal::String(payload)) = ipc_directives.get("_payload") {
                                                    // SIMULATED BARE-METAL I/O
                                                    log.push(format!("    [MOTHERBOARD I/O] Sending '{}' to Port 0x{:X} via {}", payload, port, cmd));
                                                }
                                            }
                                        }
                                        
                                        // --- INTERCEPTOR B: High-Level Hardware Interrupt (for Scheduler, NetworkSocket, etc.) ---
                                        if !is_hardware_call { // Prevent double processing
                                            if let Some(RuntimeVal::String(hw_target)) = ipc_directives.get("_target_hardware_interrupt") {
                                                is_hardware_call = true;
                                                // We can add specific logic here if needed, like for HOST_OS
                                                log.push(format!("    [HW INTERRUPT] Intercepted for {}", hw_target));
                                            }
                                        }

                                        // --- STANDARD IPC ROUTING ---
                                        // If it was NOT a direct hardware call, we proceed with normal IPC routing.
                                        if !is_hardware_call {
                                            let target = if let Some(RuntimeVal::String(t)) = ipc_directives.get("_target_mdo") {
                                                t.clone()
                                            } else { "SYS_DEV_NULL".to_string() };

                                            let action = if let Some(RuntimeVal::String(c)) = ipc_directives.get("_command") {
                                                c.clone()
                                            } else { "SYS_EVENT".to_string() };
                                            
                                            let final_ipc_payload = if let Some(RuntimeVal::Dictionary(p)) = ipc_directives.get("_payload") {
                                                p.clone()
                                            } else {
                                                let mut p = HashMap::new();
                                                for (k, v) in ipc_directives {
                                                    if !k.starts_with("_target_") && !k.starts_with("_command") {
                                                        p.insert(k.trim_start_matches('_').to_string(), v);
                                                    }
                                                }
                                                p
                                            };
                                ...
                                mdo_types::TransformStatement::EmitToRequestor { payload, .. } => {
                ...
                log.push(format!("  [KERNEL] ERROR: MDO blueprint for '{}' not found.", msg.target_id));
            }
        }
        ...
        if cycle_count > 0 {
            ...
            if let Err(e) = database::save_memory(&mut self.db_conn, &self.world_state) {
                ...
                self.app_handle.emit("kmict-state-change", ()).unwrap();
            }
        }
        log
    }
...
async fn spawn_easy_bake(app_handle: tauri::AppHandle, config: ForgeConfig, forge_path: String) -> Result<(), String> {
    ...
    let resolved_forge_path = if forge_path.starts_with("~/") {
        let home_dir = dirs::home_dir().ok_or("Could not find home directory")?;
        forge_path.replacen("~", home_dir.to_str().unwrap(), 1)
    } else {
        forge_path
    };

    let script_path = format!("{}/build_engine.py", resolved_forge_path);
    let python_bin = format!("{}/venv/bin/python3", resolved_forge_path); 
    
    // --- GUARANTEED ABSOLUTE PATHS ---
    let curr_dir = if config.curriculum_dir.starts_with("/") {
        config.curriculum_dir.clone()
    } else {
        format!("{}/{}", resolved_forge_path, config.curriculum_dir)
    };

    let out_dir = if config.output_dir.starts_with("/") {
        config.output_dir.clone()
    } else {
        format!("{}/{}", resolved_forge_path, config.output_dir)
    };
    // ---------------------------------

    println!("[FORGE] Initiating Build Engine: {}", script_path);

    let mut child = match Command::new(&python_bin)
        .arg(&script_path)
        .arg("--arch-type").arg(&config.arch_type)
        .arg("--curriculum-dir").arg(&curr_dir) 
        .arg("--output-dir").arg(&out_dir) // <--- THIS MUST BE --output-dir for build_engine.py
        // We removed --output-name because build_engine.py generates the .pth name automatically
        ...
        .arg("--max-seq-length").arg(config.max_seq_length.to_string())
        .arg("--d-model").arg(config.d_model.to_string())
        .arg("--nhead").arg(config.nhead.to_string())
        .arg("--num-encoder-layers").arg(config.num_encoder_layers.to_string())
        .arg("--num-decoder-layers").arg(config.num_decoder_layers.to_string())
        .arg("--dim-feedforward").arg(config.dim_feedforward.to_string())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .current_dir(&resolved_forge_path) 
        .spawn()
    {
        Ok(c) => c,
    ...
    thread::spawn(move || {
        use regex::Regex;
        // Regex to find tqdm speed (e.g., "1.23it/s" or "5.00s/it")
        let speed_re = Regex::new(r"(\d{1,}\.?\d{0,}|\.\d{1,})(it/s|s/it)").unwrap();
        ...
        for line in reader.lines() {
            if let Ok(l) = line {
                ...
                if l.contains("Loss:") { // Note: trainer.py outputs "Loss: 0.1234" 
                    if let Some(loss_str) = l.split("Loss:").nth(1) {
                        let clean_loss = loss_str.split(',').next().unwrap_or("").trim().to_string();
                        let _ = app_handle_out.emit("forge-telemetry-loss", clean_loss);
                    }
                }
                
                if let Some(caps) = speed_re.captures(&l) {
                    if let Some(speed_val) = caps.get(1) {
                        let _ = app_handle_out.emit("forge-telemetry-speed", speed_val.as_str().to_string());
                    }
                }
            }
        }
...
fn cmd_set_state(state: State<AppState>, key: String, val_type: String, val_str: String) -> String {
    ...
    let r_val = match val_type.as_str() {
        ...
        "String" => RuntimeVal::String(val_str.clone()),
                // --- NEW: Handle Dictionary Injection ---
        "Dictionary" => {
            // We expect val_str to be a raw JSON string like: {"pubkey": {"Float": 100.0}}
            match serde_json::from_str::<HashMap<String, RuntimeVal>>(&val_str) {
                Ok(map) => RuntimeVal::Dictionary(map),
                Err(e) => {
                    println!("[kMICT ERROR] Failed to parse Dictionary JSON: {}", e);
                    RuntimeVal::Null
                }
            }
        },
        "Array" => {
            // We expect val_str to be a raw JSON string like: [{"String": "proc_alpha"}]
            match serde_json::from_str::<Vec<RuntimeVal>>(&val_str) {
                Ok(vec) => RuntimeVal::Array(vec),
                Err(e) => {
                    println!("[kMICT ERROR] Failed to parse Array JSON: {}", e);
                    RuntimeVal::Null
                }
            }
        },
...
fn cmd_exec_mdo(
    ...
    mdo_id: String, 
    action: String, 
    payload: HashMap<String, RuntimeVal>
) -> Vec<String> {
    let mut k = state.kmict.lock().unwrap();
    // We will now determine the source_id from the payload itself
    let source_id = if let Some(RuntimeVal::String(id)) = payload.get("requestor_id") {
        id.clone()
    } else {
        "React_UI".to_string() // Default if not provided
    };

    let msg = MdoMessage {
        source_id, // Use the extracted ID
        target_id: mdo_id,
        action,
        payload,
    };
...
#[tauri::command]
fn create_user_identity(state: State<AppState>, username: String) -> Result<String, String> {
    let keypair = crate::crypto::generate_keypair();
    let user_id = format!("user_{}", username);

    let mut k = state.kmict.lock().unwrap();

    // We inject the user's core identity properties directly into the world_state
    // This simulates creating a user account in the database
    k.world_state.insert(format!("{}_id", user_id), RuntimeVal::String(user_id.clone()))?;
    k.world_state.insert(format!("{}_entity_name", user_id), RuntimeVal::String(username.clone()))?;
    k.world_state.insert(format!("{}_public_key", user_id), RuntimeVal::String(keypair.public_key))?;
    // Store the private key for the UI to fetch later (IN A REAL APP, THIS IS HIGHLY INSECURE!)
    // For our test, this simulates the user's local key storage.
    k.world_state.insert(format!("{}_private_key", user_id), RuntimeVal::String(keypair.private_key))?;
    
    Ok(format!("Created user '{}' with ID '{}'", username, user_id))
}

#[tauri::command]
fn get_user_keys(state: State<AppState>, user_id: String) -> HashMap<String, String> {
    let k = state.kmict.lock().unwrap();
    let mut keys = HashMap::new();

    if let Some(RuntimeVal::String(pub_key)) = k.world_state.get(&format!("{}_public_key", user_id)) {
        keys.insert("public_key".to_string(), pub_key.clone());
    }
    if let Some(RuntimeVal::String(priv_key)) = k.world_state.get(&format!("{}_private_key", user_id)) {
        keys.insert("private_key".to_string(), priv_key.clone());
    }
    keys
}

// And a helper command to sign the challenge from the UI
#[tauri::command]
fn sign_challenge(private_key: String, challenge: String) -> Result<String, String> {
    crate::crypto::sign_message(&private_key, &challenge)
}
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            app.manage(AppState { 
                ...
                kmict: Mutex::new(KmictKernel::new(kmict_conn, app.handle().clone())),  
            });
