# --- START OF FILE ingestion_system.py ---
...
class IngestionSystem:
    ...
    def discover_and_ingest(self):
        ...
        if not os.path.isdir(self.curriculum_root): 
            sys.exit(f"FATAL: Curriculum root not found at '{self.curriculum_root}'.")
                    ...
                    with open(filepath, 'r', encoding='utf-8') as f:
                        try:
                            if filename.endswith('.jsonl'): 
                                [self.lessons.append(json.loads(line)) for line in f]
                            else: 
                                self.lessons.append(json.load(f))
                        except json.JSONDecodeError as e: 
                            print(f"  -> WARNING: Skipping '{filename}' due to JSON error: {e}")
    ...
    def write_pre_graph_to_file(self):
        ...
        try:
            ...
            print("  -> Pre-graph saved successfully.")
        except Exception as e:
            sys.exit(f"FATAL: Failed to write to '{self.output_filepath}': {e}")

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('curriculum_root')
    parser.add_argument('output_filepath')
    args = parser.parse_args()
    
    ingest = IngestionSystem(args.curriculum_root, args.output_filepath)
    ingest.discover_and_ingest()
    ingest.write_pre_graph_to_file()
# --- END OF FILE ---
